...
This build command auto-generates `public/sitemap.xml` before Vite runs.
...
- Sitemap file is generated by `scripts/generate-sitemap.mjs` and written to `public/sitemap.xml`.
- Default canonical host in sitemap is `https://www.zollushouse.eu`.
- You can override the host during build with `SITE_URL`:

```sh
set SITE_URL=https://www.zollushouse.eu && npm run build
```

- In Google Search Console, submit:
   - `https://www.zollushouse.eu/sitemap.xml`
- If you are using a URL-prefix property, make sure the property host matches the sitemap host (`www` vs apex).
